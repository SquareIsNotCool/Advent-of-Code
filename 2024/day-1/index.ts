...
const lists = { left: new Array<number>(lines.length), right: new Array<number>(lines.length) };

for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const ids = line.split("   ");
    lists.left[i] = (parseInt(ids[0]));
    lists.right[i] = (parseInt(ids[1]));
}
...
{
    let sum = 0;
    for (const id of lists.left) {
        let index = lists.right.findIndex(x => x === id);
        if (index === -1) continue;
        
        let count = 0;
        while (lists.right[index] === id) {
            count += 1;
            index += 1;
        }

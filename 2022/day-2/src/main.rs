...
impl Outcome {
    ...
    fn shape_required_for_outcome(&self, other: &Shape) -> Shape {
        match self {
...
// const STRATEGIES: [(Shape, Strategy); _];

fn calculate_round_score(player: &Shape, opponent: &Shape) -> u32 {
    // println!(
    //     "Opponent chose {:?}! You chose {:?}",
    //     opponent,
    //     player
    // );
    player.get_value() + player.get_outcome(opponent).get_value()
}

fn part_one() {
    let total_score: u32 = STRATEGIES
        .map(|strategy|calculate_round_score(&strategy.1.to_shape(), &strategy.0))
        .iter()
        .sum();
...
fn part_two() {
    let total_score: u32 = STRATEGIES
        .map(|strategy|calculate_round_score(&strategy.1.to_outcome().shape_required_for_outcome(&strategy.0), &strategy.0))
        .iter()
        .sum();
